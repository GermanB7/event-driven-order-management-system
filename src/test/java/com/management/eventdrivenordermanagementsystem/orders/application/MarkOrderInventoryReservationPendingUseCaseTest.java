...
class MarkOrderInventoryReservationPendingUseCaseTest {

    private OrderRepository orderRepository;
    private MarkOrderInventoryReservationPendingUseCase useCase;

    @BeforeEach
    void setUp() {
        orderRepository = mock(OrderRepository.class);
        useCase = new MarkOrderInventoryReservationPendingUseCase(orderRepository);
    }

    @Test
    void marksCreatedOrderAsInventoryReservationPending() {
        UUID orderId = UUID.randomUUID();
        OrderItem item = OrderItem.create(UUID.randomUUID(), orderId, "SKU-1", 2, new BigDecimal("10.00"));
        Order order = Order.create(orderId, UUID.randomUUID(), "USD", List.of(item), Instant.parse("2026-04-01T09:00:00Z"));
        ...
        assertThat(captor.getValue().status()).isEqualTo(OrderStatus.INVENTORY_RESERVATION_PENDING);
    }
}
