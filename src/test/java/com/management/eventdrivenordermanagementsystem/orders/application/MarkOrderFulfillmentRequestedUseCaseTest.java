...
class MarkOrderFulfillmentRequestedUseCaseTest {

    private OrderRepository orderRepository;
    private MarkOrderFulfillmentRequestedUseCase useCase;

    @BeforeEach
    void setUp() {
        orderRepository = mock(OrderRepository.class);
        useCase = new MarkOrderFulfillmentRequestedUseCase(orderRepository);
    }

    @Test
    void marksConfirmedOrderAsFulfillmentRequested() {
        ...
        Order order = Order.rehydrate(
            ...
            Instant.parse("2026-04-01T09:03:00Z"),
            List.of(item)
        );
        ...
        assertThat(captor.getValue().status()).isEqualTo(OrderStatus.FULFILLMENT_REQUESTED);
    }
}
