...
@Component
public class InventoryReservationRejectedWorkflowListener {

    private static final Logger log = LoggerFactory.getLogger(InventoryReservationRejectedWorkflowListener.class);
    private static final String ORDER_AGGREGATE_TYPE = "ORDER";
    private static final String CONSUMER_NAME = "workflow-inventory-reservation-rejected-listener";
    ...
    public InventoryReservationRejectedWorkflowListener(
        ...
        this.orderCancelledCounter = meterRegistry.counter("workflow.inventory.reservation.rejected.order.cancelled");
        this.duplicateCounter = meterRegistry.counter("workflow.inventory.reservation.rejected.duplicate");
        this.failedCounter = meterRegistry.counter("workflow.inventory.reservation.rejected.failed");
    }

    @KafkaListener(
        topics = "${outbox.kafka.topic.order-events:order-events}",
        groupId = "workflow-inventory-reservation-rejected-listener",
        containerFactory = "workflowKafkaListenerContainerFactory"
    )
    @Transactional
    public void onMessage(ConsumerRecord<String, String> consumerRecord) {
        String eventType = header(consumerRecord, "eventType");
        if (!EventType.INVENTORY_RESERVATION_REJECTED.name().equals(eventType)) {
            return;
        }
        ...
        try {
            ...
            if (correlationId == null || correlationId.isBlank()) {
                correlationId = sourcePayload.path("correlationId").asText(orderId);
            }
            ...
            log.info(
                "workflow_order_cancelled_after_inventory_rejection orderId={} workflowId={} causationId={} eventType={}",
                orderId,
                workflowId,
                eventId,
                envelope.eventType()
    ...
    private ObjectNode buildPayload(
        JsonNode inventoryRejectedPayload,
        String workflowId,
        String correlationId,
        String causationId
    ) {
        ObjectNode payload = objectMapper.createObjectNode();
        payload.put("orderId", inventoryRejectedPayload.path("orderId").asText());
        payload.put("workflowId", workflowId);
        payload.put("correlationId", correlationId);
        payload.put("causationId", causationId);
        payload.put("orderStatus", "CANCELLED");

        if (inventoryRejectedPayload.hasNonNull("reason")) {
            payload.put("reason", inventoryRejectedPayload.path("reason").asText());
        }
        if (inventoryRejectedPayload.hasNonNull("failedSku")) {
            payload.put("failedSku", inventoryRejectedPayload.path("failedSku").asText());
        }
