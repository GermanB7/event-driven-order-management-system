...
@Configuration
public class WorkflowKafkaRetryDlqConfiguration {

    private static final Logger log = LoggerFactory.getLogger(WorkflowKafkaRetryDlqConfiguration.class);
    ...
    ConcurrentKafkaListenerContainerFactory<String, String> workflowKafkaListenerContainerFactory(
        ...
        @Value("${workflow.kafka.retry.max-attempts:3}") int maxAttempts,
        @Value("${workflow.kafka.retry.backoff-ms:1000}") long backoffMs,
        @Value("${workflow.kafka.dlq.topic:order-events.workflow.dlq}") String dlqTopic
    ) {
        ConcurrentKafkaListenerContainerFactory<String, String> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(consumerFactory);

        Counter retryCounter = meterRegistry.counter("workflow.consumer.retry.attempt");
        Counter dlqCounter = meterRegistry.counter("workflow.consumer.dlq");
        ...
        ConsumerRecordRecoverer meterAwareRecoverer = (consumerRecord, exception) -> {
            ...
            log.error(
                ...
                header(consumerRecord, "kafka_consumer"),
        ...
        errorHandler.addNotRetryableExceptions(NonRetryableWorkflowMessageException.class);
